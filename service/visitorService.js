...
module.exports.list = function(req,session, callback){
	...
	var args = {
  	...
  	var url =  config.api.url + "/api/visitors"+
  	  "?psize="+ psize +
  	  "&pno="+ pno;
...
module.exports.csv = function(req,session, res){
	var meCallback = function(error,data){
		if(data && data !=null){
			var fields = ['vDate','name', 'mobile','employee.name','address','description','salesStage'];
		    ...
		    fs.writeFile(fname, csv, function(err) {
		      if (err) throw err;
		      res.download(fname, "visitors-data.csv");
		    });
		}
	}
	if(req.query &&  req.query.psize){
	...
	var args = {
  	...
  	var url =  config.api.url + "/api/visitors/csv"+
  	  "?psize="+ psize +
  	  "&pno="+ pno;
  	  if(req.query && req.query.fromDate){
		url = url+"&fromDate="+req.query.fromDate
